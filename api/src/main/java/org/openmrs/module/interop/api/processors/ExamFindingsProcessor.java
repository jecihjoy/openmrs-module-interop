...
@Slf4j
@Component("interop.examFindingsProcessor")
public class ExamFindingsProcessor implements InteropProcessor<Encounter> {
	...
	public List<String> encounterTypes() {
		return Arrays.asList(Context.getAdministrationService()
		        .getGlobalPropertyValue(InteropConstant.EXAM_FINDINGS_ENCOUNTER_TYPE_UUIDS, "").split(","));
	}
	
	@Override
	public List<String> questions() {
		String complaintsString = Context.getAdministrationService()
		        .getGlobalPropertyValue(InteropConstant.EXAM_FINDINGS_CONCEPT_UUIDS, "");
	...
	public List<Observation> process(Encounter encounter) {
		...
		List<Obs> examObs = new ArrayList<>();
